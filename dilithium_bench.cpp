...
int main(int argc, char **argv) {
    ...
    if (OQS_SIG_keypair(sigalg, pk.data(), sk.data()) != OQS_SUCCESS) return 1;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        if (OQS_SIG_sign(sigalg, sig.data(), &siglen,
                         reinterpret_cast<const uint8_t*>(msg.data()), msg.size(), sk.data()) != OQS_SUCCESS) return 1;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        verify_ok = (OQS_SIG_verify(sigalg, reinterpret_cast<const uint8_t*>(msg.data()), msg.size(),
                                    sig.data(), siglen, pk.data()) == OQS_SUCCESS);
    ...
    if (mode == "tamper" && !sig.empty()) {
        sig[0] ^= 0x01;
        tamper_ok = (OQS_SIG_verify(sigalg, reinterpret_cast<const uint8_t*>(msg.data()), msg.size(),
                                    sig.data(), siglen, pk.data()) == OQS_SUCCESS);
    }
